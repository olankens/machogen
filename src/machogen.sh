...
update_android_studio() {
	...
	# Finish installation
	if [[ "$present" == "false" || 1 -eq 1 ]]; then # TODO: Remove dummy
		update_android_cmdline
...
update_lm_studio() {
	...
	# Update package
	brew install --cask --no-quarantine lm-studio
	brew upgrade --cask --no-quarantine lm-studio
...
update_spring_devtools() {
	...
	# idea installPlugins tanvd.grazi # INFO: Built-in now
