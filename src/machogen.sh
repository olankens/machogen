...
update_appearance() {
	...
	# Change wallpaper
	local address="https://github.com/olankens/codewall/raw/HEAD/src/raw/node-01.avif"
	local picture="$HOME/Pictures/Wallpapers/$(basename "$address")"
...
update_claude_code() {
	...
	if command -v studio &>/dev/null; then
		...
		"$program" installPlugins com.anthropic.code.plugin
	fi
...
update_mpv() {
	...
	# Update dependencies
	update_brew curl fileicon grep
	...
	local deposit=$(mktemp -d)
	expand_archive "$archive" "$deposit"
	"$deposit/openwith" io.mpv mkv mov mp4 avi

	# Change appearance
	sudo /usr/libexec/PlistBuddy -c "Set :CFBundleName Mpv" /Applications/mpv.app/Contents/Info.plist 2>/dev/null
	sudo /usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName Mpv" /Applications/mpv.app/Contents/Info.plist 2>/dev/null
	sudo xattr -cr /Applications/mpv.app
	sudo codesign --remove-signature /Applications/mpv.app
	sudo codesign --force --deep --sign - /Applications/mpv.app
	mv -f /Applications/mpv.app /Applications/Mpv.app
	change_appicon "mpv" "/Applications/MPV.app"
...
update_flutter_devtools() {
	...
	if command -v code &>/dev/null; then
		...
		# code --install-extension "RichardCoutts.mvvm-plus"
	...
	# Update cursor extensions
	if command -v cursor &>/dev/null; then
		cursor --install-extension "alexisvt.flutter-snippets"
		cursor --install-extension "dart-code.flutter"
		cursor --install-extension "pflannery.vscode-versionlens"
		cursor --install-extension "usernamehw.errorlens"
		# cursor --install-extension "RichardCoutts.mvvm-plus"
		# cursor --install-extension "robert-brunhage.flutter-riverpod-snippets"
	fi
...
update_git_devtools() {
	...
	if command -v cursor &>/dev/null; then
		...
		cursor --install-extension streetsidesoftware.code-spell-checker-french
	fi
...
update_ionic_devtools() {
	...
	# Update code extensions
	if command -v code &>/dev/null; then
		code --install-extension "WebNative.webnative"
	fi
...
update_react_devtools() {
	...
	if command -v code &>/dev/null; then
		code --install-extension "bradlc.vscode-tailwindcss"
		code --install-extension "dbaeumer.vscode-eslint"
		code --install-extension "deerawan.vscode-modern-react-typescript-snippets"
	...
	if command -v cursor &>/dev/null; then
		cursor --install-extension "bradlc.vscode-tailwindcss"
		cursor --install-extension "dbaeumer.vscode-eslint"
		cursor --install-extension "deerawan.vscode-modern-react-typescript-snippets"
	...
	jq '."[css][javascript][javascriptreact][json][html][md][typescript][typescriptreact][vue]"."prettier.printWidth" = 120' "$configs" | sponge "$configs"
